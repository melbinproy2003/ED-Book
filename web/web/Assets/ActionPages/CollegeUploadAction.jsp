        ...
        while (itr.hasNext())//checking if there is any value in the position like rs.next()
        {
            ...
            } else//when it is an image or multipart
            {
                ...
                if (field_name.equals("photo"))//write the name of image here it is img_product
                {
                    ...
                    String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\College\\";//path for storing image
                    Random r = new Random();//to generate random number
                    int r_num = r.nextInt(1111) + 999;//values between this are generated

                    photo = "CollegePhoto_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
...
//                        }
        ...
        String select = "select * from tbl_college where college_email='" + value[2] + "'";
...
<script type="text/javascript">
    ...
    setTimeout(function() {
        window.location.href = '../../Guest/College.jsp'
    }, 40);//40millisecend it go to next page
</script>
<%
} else {
    String InsQry = "insert into tbl_college(college_name,college_contact,college_email,college_address,college_photo,university_id,place_id,college_password)"
            + "values('" + value[0] + "','" + value[1] + "','" + value[2] + "','" + value[3] + "','" + photo + "','" + value[4] + "','" + value[6] + "','" + value[7] + "')";
