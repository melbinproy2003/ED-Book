    ...
    if (isMultipart) {
        ...
        while (itr.hasNext())//checking if there is any value in the position like rs.next()
        {
            ...
            } else//when it is an image or multipart
            {
                ...
                if (field_name.equals("post"))//write the name of image here it is img_product
                {
                    String ext = file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
                    //setting path to store image
                    File proj_path = new File(config.getServletContext().getRealPath("/"));
                    String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\Post\\";//path for storing image
                    Random r = new Random();//to generate random number
                    int r_num = r.nextInt(1111) + 999;//values between this are generated

                    photo = "Post_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
...
//                        }
    ...
    String InsQry = "insert into tbl_post(post_file,post_content,post_date,department_id,post_status)"
            + "values('" + photo + "','" + value[0] + "',curdate(),'"+session.getAttribute("departmentid")+"','" + value[1] + "')";
...
<script type="text/javascript">
    alert("Posted");
    setTimeout(function() {
        window.location.href = '../../Department/HomePage.jsp'
