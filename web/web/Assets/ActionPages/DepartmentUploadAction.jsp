        ...
        while (itr.hasNext())//checking if there is any value in the position like rs.next()
        {
            ...
            } else//when it is an image or multipart
            {
                ...
                if (field_name.equals("photo"))//write the name of image here it is img_product
                {
                    ...
                    String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\Department\\";//path for storing image
                    Random r = new Random();//to generate random number
                    int r_num = r.nextInt(1111) + 999;//values between this are generated

                    photo = "DepartmentPhoto_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
...
//                        }
        ...
        String select = "select * from tbl_department where department_email='" + value[1] + "'";
...
<script type="text/javascript">
    ...
    setTimeout(function() {
        window.location.href = '../../College/NewDepartment.jsp'
    }, 40);//40millisecend it go to next page
</script>
<%
} else {
    String InsQry = "insert into tbl_department(departmenttype_id,department_email,department_contact,department_photo,department_landmark,department_password,college_id)"
            + "values('" + value[0] + "','" + value[1] + "','" + value[2] + "','" + photo + "','" + value[3] + "','" + value[4] + "','"+session.getAttribute("collegeid")+"')";
