        ...
        while (itr.hasNext())//checking if there is any value in the position like rs.next()
        {
            ...
            } else//when it is an image or multipart
            {
                ...
                if (field_name.equals("photo"))//write the name of image here it is img_product
                {
                    ...
                    String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\Teacher\\";//path for storing image
                    Random r = new Random();//to generate random number
                    int r_num = r.nextInt(1111) + 999;//values between this are generated

                    photo = "TeacherPhoto_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
...
//                        }
        ...
        String select = "select * from tbl_teacher where teacher_email='" + value[2] + "'";
...
<script type="text/javascript">
    ...
    setTimeout(function() {
        window.location.href = '../../College/NewTeacher.jsp'
    }, 40);//40millisecend it go to next page
</script>
<%
} else {
    String InsQry = "insert into tbl_teacher(teacher_name,teacher_contact,teacher_email,teacher_photo,teacher_address,place_id,teacher_gender,teacher_dob,teacher_about,teacher_password,department_id)"
            + "values('" + value[0] + "','" + value[1] + "','" + value[2] + "','" + photo + "','" + value[3] + "','" + value[5] + "','" + value[6] + "','" + value[7] + "','" + value[8] + "','" + value[10] + "','" + value[9] + "')";
