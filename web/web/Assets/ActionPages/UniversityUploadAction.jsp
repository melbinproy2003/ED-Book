        ...
        while (itr.hasNext())//checking if there is any value in the position like rs.next()
        {
            ...
            } else//when it is an image or multipart
            {
                ...
                if (field_name.equals("photo"))//write the name of image here it is img_product
                {
                    ...
                    String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\University\\";//path for storing image
                    Random r = new Random();//to generate random number
                    int r_num = r.nextInt(1111) + 999;//values between this are generated

                    photo = "UniversityPhoto_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
...
//                        }
        ...
        String select = "select * from tbl_university where university_name='" + value[0] + "'";
        ResultSet rs = obj.selectCommand(select);
        if (rs.next()) {
%> 
<script type="text/javascript">
    alert("This university alrady exist");
    setTimeout(function() {
        window.location.href = '../../Admin/University.jsp'
    }, 40);//40millisecend it go to next page
</script>
<%
} else {
    String InsQry = "insert into tbl_university(university_name,university_photo)"
            + "values('" + value[0] + "','" + photo + "')";
