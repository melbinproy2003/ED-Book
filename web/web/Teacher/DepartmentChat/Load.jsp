...
<%
    String selQry = "select * from tbl_department_chat dc inner join tbl_department d on (dc.to_department_id=d.department_id) or (dc.from_department_id=d.department_id) where d.department_id='" + session.getAttribute("tdid") + "' order by dc_id";
    //System.out.println(selQry);
    ResultSet rs = con.selectCommand(selQry);
    while (rs.next()) {

        if (rs.getString("from_department_id").equals(session.getAttribute("tdid"))) {
...
} else if (rs.getString("to_department_id").equals(session.getAttribute("tdid"))) {
...
<%
    ...
    ResultSet z = con.selectCommand(sel);
    if (z.next()) {
        if (rs.getString("from_teacher_id").equals(session.getAttribute("tid"))) {
%>
